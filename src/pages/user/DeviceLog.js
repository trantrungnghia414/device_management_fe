...
function DeviceLog() {
    ...
    const generateWeekData = () => {
        ...
        for (
            ...
            const dayIncidents = incidents.filter((incident) => {
                ...
                return (
                    incidentDate.toDateString() === currentDate.toDateString()
                );
            });

            // Group incidents by period
            const periodIncidents = {
                Sáng: dayIncidents.filter(
                    (i) => getPeriod(i.report_time) === "Sáng"
                ),
                Chiều: dayIncidents.filter(
                    (i) => getPeriod(i.report_time) === "Chiều"
                ),
                Tối: dayIncidents.filter(
                    (i) => getPeriod(i.report_time) === "Tối"
                ),
            };
    ...
    const getStatusDisplay = (incident) => {
        ...
        if (incident.status === "completed") {
            ...
            if (
                incident.assignmentUsers &&
                incident.assignmentUsers.length > 0
            ) {
                        ...
                        {generateWeekData().map((day) => (
                            <React.Fragment key={day.date.toISOString()}>
                                {["Sáng", "Chiều", "Tối"].map(
                                    (period, periodIndex) => {
                                        const periodIncidents =
                                            day.incidents[period];

                                        return periodIncidents.length === 0 ? (
                                            <tr
                                                key={`${day.date}-${period}-empty`}
                                            >
                                                {periodIndex === 0 && (
                                                    <td
