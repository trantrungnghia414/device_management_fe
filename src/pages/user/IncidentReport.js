  ...
  return (
    <main className="container mx-auto px-4 py-8 flex flex-col flex-1">
      ...
      <div className="text-center mb-8">
        <h1 className="text-2xl font-bold mb-2">BÁO CÁO SỰ CỐ THIẾT BỊ</h1>
        <p className="text-gray-600">Vui lòng chọn vị trí phòng cần báo cáo</p>
      </div>
            ...
            {selectedBuilding.room_types.map((type) => (
              <button
                ...
                className={`room-type-btn p-4 border-2 rounded-lg transition-all ${
                  selectedRoomType?.room_type_id === type.room_type_id
                    ? "border-blue-500 bg-blue-50"
                    : "border-gray-200 hover:border-blue-500 hover:bg-blue-50"
                }`}
              >
                <p className="text-lg font-semibold">{type.room_type_name}</p>
                <p className="text-sm text-gray-500">
