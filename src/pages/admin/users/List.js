...
const List = () => {
  ...
  const handleToggleStatus = async (id, currentStatus) => {
    const action = currentStatus === 0 ? "ẩn" : "hiện";
    ...
    try {
      ...
      await Swal.fire({
        title: "Lỗi!",
        text: `Không thể ${action} người dùng`,
        icon: "error",
      });
    }
  };
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <div className="flex justify-center">
          <button
            onClick={() =>
              handleToggleStatus(params.data.id, params.data.status)
            }
            className={` rounded font-semibold ${
              params.data.status === 0
                ? "bg-green-500 hover:bg-green-600 px-4 text-white"
                : "bg-red-500 hover:bg-red-600 px-5 text-white"
            }`}
          >
            {params.data.status === 0 ? "Hiện" : "Ẩn"}
          </button>
        </div>
      ),
      sortable: true,
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <div className="flex gap-2">
          <Link to={`${config.routes.userEdit.replace(":id", params.data.id)}`}>
  ...
  return (
    <div>
      <h1 className="text-center text-2xl font-semibold">Quản lý người dùng</h1>
      <Link
        to={config.routes.userAdd}
