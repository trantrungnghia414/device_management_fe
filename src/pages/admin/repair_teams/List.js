...
const List = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get("/api/repair_teams");
    ...
    const handleDelete = async (id) => {
        ...
        try {
            // Gọi API xóa
            await axios.delete(`/api/repair_teams/${id}`);
        } catch (error) {
        ...
        },
        {
            ...
            cellRenderer: (params) => (
                <div className="flex gap-2">
                    <Link
                        to={`${config.routes.repairTeamEdit.replace(
                            ":id",
                            params.data.id
                        )}`}
                    >
    ...
    return (
        <div>
            <h1 className="text-center text-2xl font-semibold">
                Quản lý ban sửa chữa
            </h1>
            <Link
                to={config.routes.repairTeamAdd}
