...
function Edit() {
  ...
  const [initialValues, setInitialValues] = useState({ name: "", building_id: "", room_type_id: "" });
  ...
  async function fetchData() {
    try {
      ...
      setInitialValues({ name: response.data.name, building_id: response.data.building_id, room_type_id: response.data.room_type_id });
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, setErrors }) => {
      try {
        const response = await axios.put(`/api/classrooms/${id}`, values);
      ...
      <form onSubmit={formik.handleSubmit}>
        ...
        <div className="mb-6">
          ...
          {formik.touched.room_type_id && formik.errors.room_type_id && (
            <div className="text-red-500">{formik.errors.room_type_id}</div>
          )}
        </div>
