...
const List = () => {
...

    {
      ...
      cellRenderer: (params) => {
        if (params.value === "reported" || params.value === "viewed") {
          return (
            <Link
              to={config.routes.assignmentAdd.replace(":id", params.data.id)}
        ...
        const assignmentUsers = params.data.assignment?.assignment_users || [];
        ...
        return (
          <div className="bg-yellow-300 px-2 py-[-2px] rounded">Đang xử lý</div>
        );
      },
    },
    {
  ...
  return (
    <div>
      <h1 className="text-center text-2xl font-semibold">Quản lý sự cố</h1>
      <Link
                ...
                <div className="space-y-4">
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      type="text"
                      value={(() => {
                        if (!selectedRow.report_time) return "";
                        const date = new Date(selectedRow.report_time);
                        const hours = date
                          .getHours()
                          .toString()
                          .padStart(2, "0");
                  ...
                  <div className="mb-4">
                    ...
                    <input
                      type="text"
                      value={selectedRow.classroom?.name || ""}
                      readOnly
                      className="w-full px-3 py-2 border rounded-lg bg-gray-100"
                    />
                  </div>
                ...
                <div className="space-y-4">
                    ...
                    <>
                      ...
                      <div className="mb-4">
                        ...
                        <input
                          type="text"
                          value={(() => {
                            if (!selectedRow.assignment.assignment_time)
                              return "";
                            const date = new Date(
                          ...
                          {selectedRow.assignment.assignment_users?.map(
                            (assignedUser) => (
                              ...
                              >
                                ...
                                <img className="w-[500px]" src={ assignedUser.image_url} alt=""/>
                  ...
                  <div className="mb-4">
                    ...
                    <div className="w-full px-3 py-2 border rounded-lg bg-gray-100">
                      {selectedRow.status === "reported" && "Chưa phân công"}
                      {selectedRow.status === "completed" && "Đã xử lý"}
                      {selectedRow.status === "assigned" &&
                        (() => {
                          const assignmentUsers =
                            selectedRow.assignment?.assignment_users || [];
                          if (checkCompletionStatus(assignmentUsers)) {
                            return "Đã hoàn thành";
                          }
                          return "Đang xử lý";
                        })()}
                      {selectedRow.status === "processing" && "Đang xử lý"}
                      {selectedRow.status === "processed" && "Đã xử lý"}
                      {!selectedRow.status && "Chưa xử lý"}
                    </div>
                  </div>
