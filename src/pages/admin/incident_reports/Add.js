...
function Add() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        const response = await axios.post("/api/incident_reports", payload);
  ...
  const handleBuildingChange = (e) => {
    ...
    setFilteredClassrooms(selectedBuilding ? selectedBuilding.classrooms : []);
  };

  return (
    <div className="w-[50%] mx-auto p-8 mt-20 mb-36 bg-white rounded shadow-xl">
      <h1 className="text-center text-2xl font-semibold">Thêm báo cáo sự cố</h1>
      <form onSubmit={formik.handleSubmit}>
        ...
        <div className="mb-6">
          ...
          {formik.touched.building_id && formik.errors.building_id && (
            <div className="text-red-500">{formik.errors.building_id}</div>
          )}
        </div>
        ...
        <div className="mb-6">
          ...
          {formik.touched.classroom_id && formik.errors.classroom_id && (
            <div className="text-red-500">{formik.errors.classroom_id}</div>
          )}
        </div>
