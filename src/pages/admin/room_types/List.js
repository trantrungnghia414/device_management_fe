...
const List = () => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      // Gọi API xóa
      await axios.delete(`/api/room_types/${id}`);
    } catch (error) {
    ...
    },
    {
      ...
      cellRenderer: (params) => (
        <div className="flex gap-2">
          <Link
            to={`${config.routes.roomsTypeEdit.replace(":id", params.data.id)}`}
          >
  ...
  return (
    <div>
      <h1 className="text-center text-2xl font-semibold">Quản lý loại phòng</h1>
      <Link
        to={config.routes.roomsTypeAdd}
