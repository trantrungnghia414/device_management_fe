...
function Edit() {
  ...
  async function fetchData() {
    try {
      const response = await axios.get(`/api/room_types/${id}`);
      setInitialValues({ name: response.data.room_type_name });
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, setErrors }) => {
      try {
        const response = await axios.put(`/api/room_types/${id}`, values);
  ...
  return (
    <div className=" w-[50%] mx-auto p-8 mt-20 bg-white rounded shadow-xl">
      <h1 className="text-center text-2xl font-semibold">Sửa loại phòng</h1>
