...
function Edit() {
  ...
  async function fetchData() {
    try {
      ...
      setInitialValues({ name: response.data.building_name });
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, setErrors }) => {
      try {
        const response = await axios.put(`/api/buildings/${id}`, values);
  ...
  return (
    <div className=" w-[50%] mx-auto p-8 mt-20 bg-white rounded shadow-xl">
      <h1 className="text-center text-2xl font-semibold">Sửa tòa nhà</h1>
